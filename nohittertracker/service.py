...
class NoHitterTracker:
    ...
    def _event(self, event_type: str, game_details, team_id: int, message: str, is_finished: bool, snapshot: dict[str, Any], innings_pitched: float | None = None, sort_key: float = 0.0) -> TrackerEvent:
        assert self.formatter is not None
        return TrackerEvent(
    ...
    def _inspect_team(
        ...
        try:
            if snapshot['is_no_hitter']:
                ...
                if downgrade_play is not None and not is_perfect_game:
                    if downgrade_play.get('completed_innings', 0) >= alert_threshold:
                        ...
                        if downgrade_message is not None:
                            active_pitcher_stats = None if is_pitching_change else game_details.get_starting_pitcher_stats_at_threshold(team_id, alert_threshold)
                            ...
                            downgrade_events: list[TrackerEvent] = [
                                self._event('no_hitter_update', game_details, team_id, active_msg, False, snapshot, innings_pitched=alert_threshold, sort_key=self._ip_to_sort_key(alert_threshold)),
                            ]
                            downgrade_events.append(self._event('perfect_game_downgrade', game_details, team_id, downgrade_message, False, snapshot, innings_pitched=downgrade_play.get('completed_innings', alert_threshold), sort_key=downgrade_play['completed_innings'] * 3 + downgrade_play['completed_outs']))
                            if is_pitching_change:
                                ...
                                if pc_msg is not None:
                                    downgrade_events.append(self._event('pitching_change', game_details, team_id, pc_msg, False, snapshot, innings_pitched=starter_ip, sort_key=self._ip_to_sort_key(starter_ip)))
                            if is_final:
                                final_msg = self.formatter.build_no_hitter_message(game_details, team_id, is_final=True)
                                downgrade_events.append(self._event('no_hitter_update', game_details, team_id, final_msg, True, snapshot, sort_key=float('inf')))
                ...
                no_hitter_events.append(self._event('no_hitter_update', game_details, team_id, active_msg, False, snapshot, innings_pitched=alert_threshold, sort_key=self._ip_to_sort_key(alert_threshold)))
                if is_pitching_change:
                    pc_msg = self.formatter.build_pitching_change_message(game_details, team_id, starter_innings_pitched=starter_ip)
                    if pc_msg is not None:
                        no_hitter_events.append(self._event('pitching_change', game_details, team_id, pc_msg, False, snapshot, innings_pitched=starter_ip, sort_key=self._ip_to_sort_key(starter_ip)))
                if is_final:
                    final_msg = self.formatter.build_no_hitter_message(game_details, team_id, is_final=True, innings_pitched=None)
                    no_hitter_events.append(self._event('no_hitter_update', game_details, team_id, final_msg, True, snapshot, sort_key=float('inf')))
            ...
            if broken_play is not None and broken_play.get('completed_innings', 0) >= alert_threshold:
                ...
                if broken_message is not None:
                    ...
                    broken_pitcher_stats = None if is_pitching_change else game_details.get_starting_pitcher_stats_at_threshold(team_id, alert_threshold)
        ...
        *,
        ...
        for i, event in enumerate(events, 1):
            ...
            normalized_event = {
                ...
                'innings_pitched': event['innings_pitched'] if event.get('innings_pitched') is not None else snapshot.get('innings_pitched', 0),
